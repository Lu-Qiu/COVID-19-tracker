...
time_series_data <- read.csv(time_series,header =T)
  ...
  UK_time_series <- subset(time_series_data,Country.Region=='United Kingdom'& Province.State == '')
  UK_time_series <- UK_time_series[,-c(1,2,3,4)]
  UK_time_series <-as.data.frame(t(UK_time_series))
  UK_time_series$Date <- rownames(UK_time_series)
  UK_time_series$Date <- gsub("X", "", UK_time_series$Date)
  colnames(UK_time_series) <-c('Confirmed','Date')
  UK_time_series$Date <- factor(UK_time_series$Date, levels = UK_time_series$Date[order(UK_time_series$Confirmed)])
  UK_time_series$Country <- 'UK'
  
  #combine the time_series dataframes of the three countries
  time_series <- rbind(US_time_series,India_time_series,Brazil_time_series,UK_time_series)
